...
data_1 = np.loadtxt("figure_S18_NaNbO2F2_data.txt", skiprows = 1)
data_2 = np.loadtxt("figure_S18_NaTaO2F2_data.txt", skiprows = 1)

# Defining figure area and plotting data with y-errorbars on each data point

# DATA n°1
f, axis = plt.subplots(2, 2, figsize=(8, 8), sharex=True)
...
ax30.set_ylim(1.9, 2.15)

# DATA n°2
ax01.set_ylim(1.95, 2.2)
ax31.set_ylim(1.95, 2.2)
...
axis[0, 0].set_ylim(1.85, 2.1)
axis[1, 0].set_ylim(1.9, 2.15)

# DATA n°2
axis[0, 1].set_ylim(1.95, 2.2)
axis[1, 1].set_ylim(1.95, 2.2)
...
axis[1, 0].set_xlabel('T (°C)', fontsize = 18)
axis[0, 0].set_ylabel(r'Nb–F1 ($\AA$)', fontsize = 18)
axis[1, 0].set_ylabel(r"Nb–O2' ($\AA$)", fontsize = 18)

# DATA n°2
#axis[0, 1].set_title('NaTaO$_2$F$_2$', fontsize = 25, y = 1.22)
axis[0, 1].set_xlabel('T (°C)', fontsize = 18)
axis[1, 1].set_xlabel('T (°C)', fontsize = 18)
axis[0, 1].set_ylabel(r'Ta–F1 ($\AA$)', fontsize = 18)
axis[1, 1].set_ylabel(r"Ta–O2' ($\AA$)", fontsize = 18)
...
axis[1, 0].xaxis.tick_bottom() # moving the x axis tick to the top of
axis[1, 0].tick_params(which = 'major', length = 8, width = 1.5, direction = "in", labelsize = 14)
...
axis[1, 1].xaxis.tick_bottom() # moving the x axis tick to the top of
...
_,bottom = f.transFigure.inverted().transform(axis[1, 0].transAxes.transform([0,0]))
...
_,bottom = f.transFigure.inverted().transform(axis[1, 1].transAxes.transform([0,0]))
...
axis[1, 0].axline(xy1=(0, b14), slope = m14, linestyle='-', color='b', linewidth=1)
...
axis[1, 1].axline(xy1=(0, b24), slope = m24, linestyle='-', color='b', linewidth=1)
...
# DATA n°1
axis[1, 0].plot([25, 220, 220], [2.03, 2.03, 2.00], '--', color='b', linewidth=1)

# DATA n°2

# Rectangles and texts
rect_3 = mpatches.Rectangle((140, 2.04), 80, 0.03, alpha=0.25, facecolor='b')
axis[1, 0].add_patch(rect_3)

axis[1, 0].text(145, 2.045, '~0.03 $\AA$', fontsize=16, color='b')

f.savefig('figure_S18.png', dpi=300, bbox_inches='tight')
plt.show()
