...
data_file = np.loadtxt("figure_5-right_data.txt", skiprows=1)
labels = ['$f$ {$\Delta_{iso}$ ; $^{19}F$}', '$f$ {$\delta_{iso}$ ; $^{23}Na$}',
          '$f$ {$C_Q$ ; $^{23}Na$}', '$f$ {$\eta_Q$ ; $^{23}Na$}']
...
#axis.set_title("NaTaO$_2$F$_2$", fontsize=36, y=1.1, x = -0.25)
#axis.legend(loc='center right', bbox_to_anchor=(1.3, 1.1), fontsize=18)

fig.savefig('figure_5-right.png', dpi=300, bbox_inches='tight')
plt.show()
